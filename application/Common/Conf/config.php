...
if(file_exists(SITE_PATH."data/conf/config.php")){
	$runtime_config=include SITE_PATH."data/conf/config.php";
}else{
    $runtime_config=array();
}

if (file_exists(SITE_PATH."data/conf/route.php")) {
    $routes = include SITE_PATH.'data/conf/route.php';
} else {
    $routes = array();
}

if (file_exists(SITE_PATH."data/conf/caches.php")) {
    $caches = include SITE_PATH.'data/conf/caches.php';
} else {
    $caches = array();
}
...
$configs= array(
        ...
        /* 标签库 */
        'TAGLIB_BUILD_IN' => CMS_CORE_TAGLIBS,
        ...
        'APP_AUTOLOAD_LAYER'    =>  'Controller,Model,Logic', // 模块自动加载的类库后缀
...
if(defined('MODE_NAME') && MODE_NAME == 'cli' && file_exists(SITE_PATH."data/conf/cli.php")) {
    $cli_configs = include SITE_PATH.'data/conf/cli.php';
}

return  array_merge($configs, $db, $caches, $runtime_config, $cli_configs);
