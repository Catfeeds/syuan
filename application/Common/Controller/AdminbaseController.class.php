...
class AdminbaseController extends AppframeController {
	...
	public function __construct() {
        $this->ismobile = sp_is_mobile();
        $this->isweixin = sp_is_weixin();

        $admintpl_path = C("SP_ADMIN_TMPL_PATH");

        $this->theme = C("SP_ADMIN_DEFAULT_THEME");
        if($this->ismobile) {
            $this->theme = C("SP_ADMIN_MOBILE_THEME");
        }
        $admintpl_path .= $this->theme."/";
    ...
    function _initialize(){
		...
		if($id > 0) {
			...
			if(empty($referer)) {
				$referer = I('server.REQUEST_URI', '', 'trim');
			}
			$this->assign("referer", html_entity_decode($referer));
