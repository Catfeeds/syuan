...
function sp_create_admin_account($db,$table_prefix,$authcode){
    ...
    $password = md5(md5(I("post.manager_pwd")).$salt);
    $email = I("post.manager_email");
    $create_date = date("Y-m-d h:i:s");
    $ip = get_client_ip(0, true);
    $sql =<<<hello
    INSERT INTO `{$table_prefix}users` 
    (id,user_login,user_pass,pass_salt,user_nicename,user_email,create_time,user_activation_key,user_status,user_type,last_login_ip,last_login_time) VALUES 
    ('1', '{$username}', '{$password}', '{$salt}', 'admin', '{$email}', '{$create_date}', '', '1', '1','{$ip}','{$create_date}');;
hello;
    $db->execute($sql);
