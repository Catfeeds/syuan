...
class PublicController extends AdminbaseController {
    ...
    public function login() {
    	if(sp_get_current_admin_id() > 0){//已经登录
    ...
    public function dologin(){
    	...
    	if(!sp_check_verify_code()){
    		...
    		if(!empty($result) && $result['user_type']==1) {
    			if(md5($pass.$result['pass_salt']) == $result['user_pass']){
					...
					session('last_login', array('time' => $result["last_login_time"], 'ip' => $result['last_login_ip']));
    				$result['last_login_ip']=get_client_ip(0,true);
    				$result['last_login_time']=date("Y-m-d H:i:s");
    				$user->save($result);
    				cookie("admin_username", $name, time()+30*24*3600);
    				$this->success(L('LOGIN_SUCCESS'),U("Index/index"));
    			}else{
    				$this->error(L('PASSWORD_NOT_RIGHT'));
    			}
    		}else{
                M('UserFailedlogin')->add(array(
