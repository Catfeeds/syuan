...
class UserController extends AdminbaseController{
	...
	function index(){
		...
		foreach($users as $key => $user) {
			$user['role_ids'] = $role_user_model->where(array('user_id' => $user['id']))->order('role_id asc')->getField('role_id', true);
			$users[$key] = $user;
		}

		$roles = $this->role_model->getField('id,name');
		...
		if(IS_POST){
			if(!empty($_POST['role_id']) && is_array($_POST['role_id'])){
				...
				if($this->users_model->create()) {
					...
					$this->users_model->user_type = 1;
					$this->users_model->pass_salt = sp_random_string(6);
					$this->users_model->user_pass = md5(md5($this->users_model->user_pass).$this->users_model->pass_salt);
	...
	function userinfo_post(){
		if (IS_POST) {
			...
			->field("user_login,mobile,user_email,last_login_ip,last_login_time,create_time,user_activation_key,user_status,user_type",true)//排除相关字段
			->create();
			if ($create_result) {
