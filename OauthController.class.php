...
class OauthController extends HomebaseController {
				...
				if($wechatmp) {
					if($wechatmp['type'] == 3) {
						...
						if($user->getId()) {
							...
							$user_info = array('name' => $userArray['name'], 'head' => $userArray['avatar']);
	...
	private function _login_handle($user_info, $type, $token, $fromuid = 0) {
		...
		if($find_oauth_user && $find_oauth_user['status'] == 1) {
			...
			if($find_user) {
				if($find_user['user_status'] == '0'){
					...
					$this->_refresh_oauth_user($type, $user_info, $token, $find_oauth_user['uid'], $find_oauth_user['id']);
					...
					if($user_info['head'] && 
						(!$find_user['avatar'] || 
							(substr($find_user['avatar'], 0, 12) == substr($user_info['head'], 0, 12) &&
								$find_user['avatar'] != $user_info['head']
							)
						)
					) {
						$login_data['avatar'] = $user_info['head'];
					}
		...
		//while($users_model->where(array('user_login' => $user_login))->count() > 0) {
